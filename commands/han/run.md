...
> **前置安全檢查（主代理在產生任何 shell 指令前必做）**：若 `$ARGUMENTS` 非空，先自行確認它**完全符合** epic id 格式 `^[0-9a-f]{8}$`（8 個小寫十六進位字元，無空白、無 shell 特殊字元）。
> - 不符 → **立即停止**，回報「epic id 格式不合法」，**絕不可**把該值放進任何 Bash 指令（避免命令注入）。
> - 符合（或為空，代表自動選最新）→ 才可繼續，將該已驗證的值嵌入下方指令。
